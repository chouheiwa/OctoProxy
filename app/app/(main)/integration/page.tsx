...
const electronKeyApi = {
  get: async () => {
    const response = await fetch(`${API_BASE}/electron-key`, {
      headers: getAuthHeaders(),
    });
    if (!response.ok) {
      throw new Error("Failed to fetch electron key");
    }
    return response.json();
  },
  regenerate: async () => {
    const response = await fetch(`${API_BASE}/electron-key/regenerate`, {
      method: "POST",
      headers: getAuthHeaders(),
    });
    if (!response.ok) {
      throw new Error("Failed to regenerate electron key");
    }
    return response.json();
  },
};

interface ElectronKey {
  key: string;
  keyPrefix: string;
  isActive: boolean;
}
...
export default function Integration() {
  ...
  const [electronKey, setElectronKey] = useState<ElectronKey | null>(null);
