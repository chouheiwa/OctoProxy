...
function parseBracketToolCalls(responseText) {
  ...
  for (let i = 0; i < callPositions.length; i++) {
    ...
    if (bracketEnd !== -1) {
      ...
      if (lastBracket !== -1) toolCallText = segment.substring(stBracket + 1);
      else continue;
    }
...
export class KiroService {
   ...
   * 流式 API 调用
   */
  async *streamApi(model, body, isRetry = false, retryCount = 0) {
    if (!this.isInitialized) await this.initialize();
    ...
    try {
      ...
      if (error.response?.status === 403) {
        ...
        try {
          ...
          console.error(
            "[Kiro] Token refresh failed getUsageLimits retry:",
